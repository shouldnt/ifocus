...
HomeSlide.prototype.initEvent = function () {
	...
	$('.js-slide-toggle').click(function (e) {
		...
		if (_this.isSlideActive) {
			$('.page-ham').addClass('active');
			$(document).trigger("HIDE_SLIDE");
		} else {
			$('.page-ham').removeClass('active');
...
HomeSlide.prototype.prevSlide = function () {
	...
	var prevNavContent = prev - 1 < 0 ? _this.$slides[_this.$slides.length - 1].querySelector('.nav-content').innerHTML : _this.$slides[prev - 1].querySelector('.nav-content').innerHTML;
		...
		(function () {
			...
			for (var i = 0; i < 2; i++) {
				...
				img.onload = function (_index, _img) {
					var texture = _this.textureArr[_index];
					texture.image = _img;
...
HomeSlide.prototype.nextSlide = function () {
	...
	var nextNavContent = next + 1 >= _this.$slides.length ? _this.$slides[0].querySelector('.nav-content').innerHTML : _this.$slides[next + 1].querySelector('.nav-content').innerHTML;
...
HomeSlide.prototype.animateOut = function () {
	...
	img.onload = function (_img) {
		var texture = _this.textureArr[0];
		texture.image = _img;
		texture.needsUpdate = true;
...
HomeSlide.prototype.canvas = function () {
	...
	// デモに使用する画像URL
	var assetUrls = [_this.$images[0].getAttribute('src'), _this.$images[0].getAttribute('src'), document.querySelector('.distor-img').getAttribute('src')];
 ...
 */
	...
	// テクスチャの初期化
	assetUrls.forEach(function (url, index) {
		var img = new Image();
...
VideoFrame.prototype.initEvent = function () {
	...
	$('.js-start-player').click(function (e) {
		...
		var videoObject = _utils2.default.parseVideo(videoUrl);
		var embedLink = '';
